...
class IntroFY:
    def __init__(self):
        ...
        self.tool = None
        print("Grammar checker disabled — using fallback grammar scoring.")
        ...
        # Semantic Anchors: We compare user text against these concepts vectors
        self.concepts = {
    ...
    def calculate_speech_rate(self, word_count, duration_sec):
        ...
        if duration_sec <= 0: return 0, "Invalid duration (0s)"
    ...
    def calculate_grammar(self, text, word_count):
        """
        Grammar scoring fallback for systems where LanguageTool fails.
        This prevents crashes due to LanguageTool zip/API errors.
        """

        # If LanguageTool disabled → fallback logic
        if self.tool is None:
            suggestions = []
            if word_count == 0:
                return 0, "Empty text provided."

            # Heuristic 1 — detect long sentences
            sentences = sent_tokenize(text)
            avg_len = word_count / max(len(sentences), 1)

            score = 10  # start from perfect
            
            if avg_len > 25:
                score -= 2
                suggestions.append("Your sentences are long. Try splitting them for clarity.")

            # Heuristic 2 — check lowercase 'i'
            if " i " in text:
                score -= 2
                suggestions.append("Capitalize 'I' when referring to yourself.")

            # Heuristic 3 — missing capitalization at sentence start
            bad_starts = sum(1 for s in sentences if s and s[0].islower())
            if bad_starts > 0:
                score -= 2
                suggestions.append("Some sentences start with lowercase letters.")

            # Ensure score stays in [2, 10]
            score = max(2, min(score, 10))

            if not suggestions:
                suggestions.append("Fallback grammar check: No major issues detected.")
    ...
    def calculate_vocabulary(self, text, word_count):
        ...
        if not words: return 0, "No valid words"
        
        unique_words = set(words)
        ttr = len(unique_words) / len(words)
        
        # CSV Thresholds
        if 0.9 <= ttr <= 1.0: score = 10
        elif 0.7 <= ttr <= 0.89: score = 8
        elif 0.5 <= ttr <= 0.69: score = 6
        elif 0.3 <= ttr <= 0.49: score = 4
        else: score = 2
    ...
    def calculate_clarity(self, text, word_count):
        ...
        # CSV Thresholds
        if 0 <= rate <= 3: score = 15
        elif 4 <= rate <= 6: score = 12
        elif 7 <= rate <= 9: score = 9
        elif 10 <= rate <= 12: score = 6
        else: score = 3
    ...
    def calculate_engagement(self, text):
        ...
        # We use VADER 'pos' score as a proxy for "probability of positive words"
        scores = self.sentiment_analyzer.polarity_scores(text)
        metric = scores['pos']
        compound = scores['compound']
        
        # Logic adjustment: If text is highly positive overall (compound > 0.8), 
        # we treat it as high engagement even if specific positive word count is low.
        final_metric = max(metric, compound)
        
        # Logic Table
        if final_metric >= 0.9: score = 15
        elif 0.7 <= final_metric <= 0.89: score = 12
        elif 0.5 <= final_metric <= 0.69: score = 9
        elif 0.3 <= final_metric <= 0.49: score = 6
        else: score = 3 
    ...
    def check_content_structure(self, text):
        ...
        # Check first 50 chars for salutation
        ...
        # Strategy: Look for "My name is" OR Spacy PERSON entity
        ...
        # C. FLOW (5 Marks)
        # Order: Salutation -> Name -> Closing
        # Simple heuristic: Salutation at start, 'Thank' at end
    ...
    def check_semantic_similarity(self, text, anchor_phrases):
        ...
        # We encode the user text and the anchor phrases
        # We only check sentences that might be relevant to save time, 
        # but for short transcripts, encoding the whole text sentence-by-sentence is fine.
        
        doc_sentences = sent_tokenize(text)
        if not doc_sentences: return False
        ...
        # Result is a matrix of scores [num_sentences x num_anchors]
        cosine_scores = util.cos_sim(doc_embeddings, anchor_embeddings)
        
        # If the best match is > 0.35, we consider it a hit
    ...
    def analyze(self, transcript_text, duration_sec):
        ...
        return {
            ...
            "breakdown": [
                ...
                {"criteria": "Content Relevance", "score": s_cont, "max": 30, "feedback": f"Found: {', '.join(structure_data['detected_keywords'])}"},
